...
public class CreateNewProjectCommand : IStreamRequest<CreatedNewProjectResponse>
{
    ...
    public class CreateNewProjectCommandHandler : IStreamRequestHandler<CreateNewProjectCommand, CreatedNewProjectResponse>
    {
        ...
        )
        {
            ...
            response.LastOperationMessage = "Starter project has been cloned from 'https://github.com/archolet/Archolet'.";
        ...
        {
            // Download zip on url
            string releaseUrl = "https://github.com/archolet/Archolet/archive/refs/tags/v1.2.0.zip";
            ...
            File.Delete(zipPath);
            Directory.Move(
                sourceDirName: $"{Environment.CurrentDirectory}/Archolet-1.2.0",
        ...
        private async Task renameProject(string projectName)
        {
            ...
            await replaceFileContentWithProjectName(
                path: $"{Environment.CurrentDirectory}/Archolet.sln",
                search: "Archolet",
                projectName: projectName.ToPascalCase()
            );
            await replaceFileContentWithProjectName(
                path: $"{Environment.CurrentDirectory}/Archolet.sln.DotSettings",
                search: "Archolet",
                projectName: projectName.ToPascalCase()
            );
        ...
        private async Task removeSecurityMechanism(string projectName)
        {
                ...
                predicate: line =>
                    (
                        new[]
                        {
                            ...
                            "using Archolet.Core.Security.DependencyInjection;",
            ...
            await FileHelper.RemoveLinesAsync(
                ...
                predicate: line =>
                    (new[] { "<PackageReference Include=\"Archolet.Core.Security.DependencyInjection\" Version=\"1.0.0\" />", }).Any(
                        line.Contains
                    )
            );
            await FileHelper.RemoveLinesAsync(
                ...
                predicate: line =>
                    (new[] { "<PackageReference Include=\"Archolet.Core.Security\" Version=\"1.1.1\" />" }).Any(line.Contains)
            );
            await FileHelper.RemoveLinesAsync(
            ...
            await FileHelper.RemoveContentAsync(
                ...
                contents: new[] { "<PackageReference Include=\"Archolet.Core.Security.WebApi.Swagger\" Version=\"1.0.0\" />;", }
            );
        }
        ...
        private async Task initializeGitRepository(string projectName)
        {
            ...
            await GitCommandHelper.CommitChangesAsync("chore: initial commit from Archolet.Gen");
            Directory.SetCurrentDirectory("../");
        }
    }
}
